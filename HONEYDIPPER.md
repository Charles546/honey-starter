...
- **Image-tag bump freeze-exception (routine, maintained class):** `deploy/docker-compose.yaml` (the `image: ${HONEYDIPPER_IMAGE:-...}` daemon ref and the `image: ${HD_UI_IMAGE:-...}` ui ref) and `.env.example` (the commented `# HONEYDIPPER_IMAGE=...` / `# HD_UI_IMAGE=...` lines) may be edited to bump the pinned Docker Hub image tags. These two files remain frozen for everything else; the image pins are a maintained/replaceable class of edit.
